...

{
  ...
  "description": "Tag to implement TikTok pixel and send request to server-side tag",
  "containerContexts": [
    "WEB"
  ]
}
...
const sendEvent = function(){
  ...
  // Getting the event parameters, if any
  ...
  // Getting the ttq object
  var pixels = copyFromWindow('ttq._i');
  log('pixels before event sending:',pixels);

  // Sending the event
  data.pixelId.split(',').forEach(function(thisPixelId){

    // Initiating the ttq object for thisPixelID, if necessary  
    if (!pixels || pixels.indexOf(thisPixelId)==-1 || !pixels[thisPixelId]._init){
      data.pixelId.split(',').forEach(function(thisPixelId){
        ...
        injectScript(url,log('success'),data.gtmOnFailure);
      }); 
    }
    pixels = copyFromWindow('ttq._i');
    log('pixels before event sending:',pixels);

    if (eventName=='PageView'){
      callInWindow('ttq.instance',thisPixelId).page();
    } else {
      if (eventParams){
        callInWindow('ttq.instance',thisPixelId).track(eventName, eventParams);
      } else {
        callInWindow('ttq.instance',thisPixelId).track(eventName);
      }
    }
  });
};
